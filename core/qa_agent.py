...
class QAAgent:
    ...
    def _build_source_manifest(self) -> list:
        ...
        if dashboards:
            ...
            if orphans:
                # Create individual manifest entries for each orphan worksheet
                # so they match the individual pages generated by the PBIP
                # generator.  Previously all orphans were collapsed into a
                # single "Additional Sheets" entry whose page name never
                # matched the generated pages, causing the entire entry to
                # score 0.
                for ws in orphans:
                    ws_fields = _extract_fields_for_ws(ws)
                    manifest.append({
    ...
    # Tableau mark type -> set of acceptable Power BI visual types.
    # An EMPTY set means "accept any type" (e.g. "automatic").
    # Keys are lowercased; lookup normalises the incoming mark type.
    _MARK_TO_PBI = {
        ...
        "gantt-bar": {"clusteredBarChart", "tableEx"},
        "polygon": {"filledMap", "shapeMap"},
        "multipolygon": {"filledMap", "shapeMap"},
        # Map variants — all treated as filledMap/map interchangeable
        "map": {"map", "filledMap", "shapeMap"},
        "filled-map": {"filledMap", "map", "shapeMap"},
        "filledmap": {"filledMap", "map", "shapeMap"},
        "filledmap": {"filledMap", "map", "shapeMap"},
        "shape": {"tableEx", "matrix"},
        "density": {"scatterChart"},
        "heatmap": {"matrix"},
        "kpi": {"cardVisual", "kpi", "card"},
        "ban": {"cardVisual", "kpi", "card"},
        "automatic": set(),  # empty set = accept any type
    }

    def _tableau_mark_to_pbi_visual(self, mark_type: str) -> set:
        """Return set of acceptable PBI visual types for a Tableau mark.

        Returns an empty set for unknown/automatic types, which the caller
        treats as "accept anything".  Always returns a set, never a string.
        """
        val = self._MARK_TO_PBI.get(mark_type.lower().strip())
        if val is None:
            return set()
    ...
    def _compute_fidelity(self, live_result, source_manifest=None) -> dict:
        ...
        for src_idx, src_tab in enumerate(source_manifest):
            ...
            for v in visuals:
                ...
                # queryRef formats:
                #   "Data.Region"          -> "region"
                #   "Sum(Data.Sales)"      -> "sales"  (remove agg + parens)
                #   "CountDistinct(Data.X)" -> "x"
                for f in v.get("queryRefs", []):
                    import re as _re
                    # Strip aggregation wrapper e.g. Sum(Data.Sales) -> Data.Sales
                    f_clean = _re.sub(
                        r'^[A-Za-z]+\((.+)\)$', r'\1', f.strip()
                    )
                    if "." in f_clean:
                        # "Data.Sales" -> "sales"
                        pbi_fields.add(f_clean.split(".")[-1].lower().strip("() "))
                    else:
                        pbi_fields.add(f_clean.lower().strip("() "))
    ...
    # NOTE: _MARK_TO_PBI is defined earlier in the class (set-based version).
    # This secondary single-value lookup is used only by _pre_publish_structural_qa
    # for the "expected_pbi_type" field in its per-tab manifest entries.
    _MARK_TO_PBI_SINGLE = {
    ...
    def _pre_publish_structural_qa(self, spec, config, pbip_dir):
        ...
        for tab in visible_tabs:
            ...
            if tab["type"] == "worksheet":
                ...
                entry["expected_pbi_type"] = self._MARK_TO_PBI_SINGLE.get(
                    ...
                    try:
                        ...
                        for role_data in qs.values():
                            for proj in role_data.get("projections", []):
                                ...
                                if nr:
                                    # Strip aggregation prefixes from
                                    # nativeQueryRef: "Sum of Sales" -> "sales"
                                    # Also handle: "Count of X", "Min of X",
                                    # "Max of X", "Count Distinct of X", etc.
                                    import re as _re
                                    clean = _re.sub(
                                        r'^(sum|avg|average|count distinct|'
                                        r'count|min|max|median)\s+of\s+',
                                        '',
                                        nr.lower().strip(),
                                    )
